...
@click.option("--frozen-holdout", default="./frozen_holdout_50.json")
@click.option("--out-dir", default="./bert_hpc")
def main(csv_input: str, frozen_holdout: str, out_dir: str) -> None:
    ...
    src = build_model_source_from_single(df_raw, product_col="nama produk")
    frozen = json.load(open(frozen_holdout, encoding="utf-8"))
    split = run_v5_split(
        src, product_col="nama produk", text_col="text",
    ...
    syn = generate_synthetic_dataset(dtr, text_col="text",
                                     total_data=v5.synthetic_total, seed=config.seed)
    pool = build_final_training_pool(dtr, syn, dva, dho, text_col="text")
    audit_final_pool_no_leakage(pool["X_train_text"], pool["X_val_text"],
                                pool["X_holdout_text"])

    def _save(df: pd.DataFrame, name: str, cols=("text", "label_id")) -> str:
        out = df.copy()
        out["label"] = out["label_id"].map({0: "safe", 1: "unsafe"})
        keep = ["nama produk", "text", "label", "label_id", "gold_label",
                "group_key"]
    ...
    syn_out = syn[["text", "label", "label_id", "synthetic_rule"]].copy()
    syn_out.to_csv(os.path.join(out_dir, "synthetic.csv"), index=False)

    combined = pd.concat([
        dtr[["text", "label_id"]].assign(label=dtr["label_id"].map({0: "safe", 1: "unsafe"})),
        syn_out[["text", "label", "label_id"]],
